...
export default class City extends Component {
  ...
  state = {
    ...
    columns: [
      ...
      {title: '坐标值', dataIndex: 'value', align: 'center'},
  ...
  onAddFinish = async values => {
    const res = await CityApi.ReqAdd({...values})
    if (res && res.code === 0) {
  ...
  onUpdateFinish = async values => {
    const res = await CityApi.ReqUpdate({id: this.state.curr_id, ...values})
    if (res && res.code === 0) {
  ...
  delSure = async record => {
    const res = await CityApi.ReqDelete({id: record._id})
    if (res.code === 0) {
  ...
  render () {
    ...
    return (
      <div className='trech-wrap'>
        <div className='wrap-title'>城市信息</div>
        ...
        >
          <Form
            ...
            <Form.Item
              ...
              rules={[{ required: true, message: 'please input your value!'}]}
            >
              <Input placeholder="输入X,Y坐标值中间用‘，’隔开！" />
            </Form.Item>
